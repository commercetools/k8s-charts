...
yamllinter() {
  ...
  # If values.yaml files exist in folder "values/"", lint them.
  if [ -d $1/values ]; then
    for file in $(find $1/values -name '*.yaml'); do
      printf "Linting $(basename "${file}") at $(dirname "${file}")\n"
      run yamllint -c test/circle/lintconf.yml $file
    done
  fi
}
...
# Verify that chart folows our best practices
bestpracticelinter() {
  # If we have a deployment - we should have a livenessProbe in it
  egrep -r "^kind: Deployment" -l "$1/templates" | while IFS= read -r deployment
  do 
    printf "Linting $(basename "${deployment}") at $(dirname "${deployment}")\n"
    grep 'livenessProbe:' "$deployment" >/dev/null || ( echo "$deployment has no livenessProbe" && exitCode=1 )
  done
}

git remote add k8s git@github.com:commercetools/k8s-manifests.git
...
for directory in ${CHANGED_FOLDERS}; do
  if [ "$directory" == "incubator/common" ]; then
    ...
    printf "\nUpdating dependencies on the chart at ${directory}\n"
    run helm dependency update ${directory}
